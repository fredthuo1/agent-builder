...
app.get("/api/tasks", (_req, res) => {
  db.all("SELECT id, title, description, priority, status, due_date, is_completed FROM tasks ORDER BY id DESC", (err, rows) => {
...
app.post("/api/tasks", (req, res) => {
  const body = req.body || {};
  const cols = ["title","description","priority","status","due_date","is_completed"];
  const fields = [{"name":"title","type":"text","required":true},{"name":"description","type":"text","required":false},{"name":"priority","type":"select","options":["Low","Medium","High"],"required":true},{"name":"status","type":"select","options":["Todo","In Progress","Done"],"required":true},{"name":"due_date","type":"date","required":false},{"name":"is_completed","type":"boolean","required":true}];
  ...
  db.run(
    ...
    function (err) {
      if (err) return res.status(500).json({ error: err.message });
      db.get("SELECT id, title, description, priority, status, due_date, is_completed FROM tasks WHERE id = ?", [this.lastID], (e2, row) => {
