...
export default function Page() {
  ...
  return (
    <main style={{ maxWidth: 980, margin: "0 auto", padding: 24 }}>
      <h1 style={{ fontSize: 28, fontWeight: 900, margin: 0 }}>Build A Mini Crm For Contacts Wi</h1>
      <p style={{ opacity: 0.7, marginTop: 8 }}>Full-stack app generated from: "Build a mini CRM for contacts with fields: name, email, phone, company, stage (lead, qualified, customer), notes."</p>

      {err && (
        <div style={{ marginTop: 12, padding: 10, border: "1px solid #f2caca", background: "#fff7f7", borderRadius: 12, fontSize: 12 }}>
          <strong>Error:</strong> {err}
        </div>
      )}

      <section style={{ marginTop: 14, padding: 14, border: "1px solid #eee", borderRadius: 14 }}>
        <div style={{ fontWeight: 800, marginBottom: 10 }}>Create {pretty(SINGULAR)}</div>
        ...
        <div style={{ display: "grid", gap: 10 }}>
          {items.map((it) => (
            <div key={it.id} style={{ border: "1px solid #eee", borderRadius: 14, padding: 12, display: "flex", justifyContent: "space-between", gap: 12 }}>
              <div style={{ display: "grid", gap: 4 }}>
                ...
                {FIELDS.map((f) => (
                  <div key={f.name} style={{ fontSize: 13 }}>
                    <strong style={{ fontSize: 12 }}>{pretty(f.name)}:</strong>{" "}
              ...
              <button
                onClick={() => del(it.id)}
                style={{ height: 32, padding: "6px 10px", borderRadius: 10, border: "1px solid #ddd", background: "#fff", fontSize: 12 }}
              >
                Delete
              </button>
            </div>
